# AI Brain Index

This index points to the most important AI Brain and repository context files.

Regenerate the local repository index with:

```bash
npm run brain:index
```

Search AI Brain and project context with:

```bash
npm run brain:search -- "query"
```

Create pre-implementation impact guidance with:

```bash
npm run brain:impact -- "change description"
```

Run local AI Brain governance health checks with:

```bash
npm run brain:health
```

## Start Here

- `../README.md`
- `../agent-start.md`
- `../adapters/README.md`
- `../governance/source-of-truth-map.md`
- `../governance/security-preflight.md`
- `../governance/developer-onboarding.md`
- `../governance/validation-profiles.md`
- `../knowledge/agent-session-start.md`
- `../planning/AI_BRAIN_PRO_DISCOVERY_REPORT.md`
- `../../../AGENTS.md`

## Adapters

- `../adapters/README.md`
- `../adapters/codex.md`
- `../adapters/generic-agent.md`

## Governance

- `../governance/source-of-truth-map.md`
- `../governance/artifact-lifecycle-policy.md`
- `../governance/review-finding-registry.md`
- `../governance/security-preflight.md`
- `../governance/health-checks.md`
- `../governance/validation-profiles.md`
- `../governance/retrieval-contracts.md`
- `../governance/memory-integrity-model.md`
- `../governance/developer-onboarding.md`

## Knowledge

- `../knowledge/project-overview.md`
- `../knowledge/product-decisions.md`
- `../knowledge/architecture-principles.md`
- `../knowledge/sdlc-flow.md`
- `../knowledge/module-catalog.md`
- `../knowledge/testing-map.md`
- `../knowledge/known-risks.md`

## Harness

- `../loop-harness/goal-contract-template.md`
- `../loop-harness/how-to-write-good-goals.md`
- `../loop-harness/codex-setup.md`
- `../loop-harness/validator-rules.md`
- `../loop-harness/maker-checker-flow.md`
- `../loop-harness/checker-review-checklist.md`
- `../loop-harness/permissions-policy.md`
- `../loop-harness/worktree-policy.md`
- `../loop-harness/automation-policy.md`
- `../loop-harness/stop-conditions.md`
- `../loop-harness/cost-guardrails.md`

## Goal Templates

- `../templates/goal-contract-template.md`
- `../templates/goal-feature-template.md`
- `../templates/goal-refactor-template.md`
- `../templates/goal-bugfix-template.md`
- `../templates/goal-docs-template.md`
- `../templates/goal-test-template.md`
- `../templates/checker-review-template.md`

## Context Packs

- `../context-packs/README.md`
- `../templates/context-pack-template.md`
- `../scripts/create-context-pack.mjs`
- `../governance/artifact-lifecycle-policy.md`

## Generated Repo Index

- `repo-map.json`
- `file-catalog.md`
- `module-map.md`
- `../scripts/create-repo-index.mjs`
- `../scripts/search-brain.mjs`
- `../scripts/analyze-impact.mjs`
- `../scripts/health-check.mjs`

## Memory

- `../memory/open-decisions.md`
- `../memory/implementation-history.md`
- `../memory/sprint-summaries/README.md`
- `../memory/memory-update-checklist.md`
- `../governance/memory-integrity-model.md`

## Source Documents

- `../../../project-context/PRODUCT_BRIEF.md`
- `../../../project-context/TECH_STACK.md`
- `../../../project-context/SECURITY_PRIVACY.md`
- `../../../project-context/TEST_STRATEGY.md`
- `../../../docs/product/PRD.md`
- `../../../docs/architecture/ARCHITECTURE.md`
- `../../../docs/VERIFY_MATRIX.md`

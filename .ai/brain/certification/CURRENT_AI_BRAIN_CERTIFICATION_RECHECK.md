...
| Branch protection requires every newly documented epic gate. | NOT YET RESOLVED | `gh api repos/Vescik/makdolan-mvp/branches/main/protection/required_status_checks` returned only `verify`. `epic_pr_codex_gate` should be added only after PR #8 merges `.github/workflows/epic-pr-review.yml` to `main`; policy documents this bootstrap sequence. |
| New GitHub Actions workflows have run remotely. | PARTIALLY RESOLVED | PR #8 (`https://github.com/Vescik/makdolan-mvp/pull/8`) triggered the new `Epic PR Codex gate` workflow on head SHA `f43b6358b5414554104bdd24c3d06bf918b07492`; `epic_pr_codex_gate` passed. The final certification and create-release workflows are manual and cannot be meaningfully run until a real completed epic exists. |
...
| Publication PR exists for these governance changes. | RESOLVED | Draft PR #8 is open: `https://github.com/Vescik/makdolan-mvp/pull/8`. The remote head is `codex/epic-release-governance-gates` at SHA `f43b6358b5414554104bdd24c3d06bf918b07492`. |
| Current PR remote checks are complete. | PARTIALLY RESOLVED | On PR #8, `verify` passed, `epic_pr_codex_gate` passed, GitGuardian passed, and the non-epic `epic_pr_ci`/`epic_pr_codex_review` jobs skipped as designed. The existing `codex-review` check remains in progress at the time of this recheck. |
...
| GitHub Actions workflows have not been executed remotely in this local recheck. | Partially resolved: `Epic PR Codex gate` ran remotely on PR #8 and `epic_pr_codex_gate` passed. Existing `verify` also passed. | Wait for the existing `codex-review` check to complete, merge PR #8 through branch protection, then add `epic_pr_codex_gate` to required branch protection checks. Run final certification/release workflows only for a real completed epic. |
| Governance changes are not published yet. | Resolved to PR state: draft PR #8 is open against `main`; it has not merged yet. | Wait for checks and review, mark ready when appropriate, merge through branch protection, then add `epic_pr_codex_gate` to required branch protection checks. |
